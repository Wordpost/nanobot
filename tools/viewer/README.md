...
Просто запусти скрипт. Он автоматически проверит зависимости, выполнит `npm run build` для фронтенда и поднимет FastAPI бэкенд на порту 2004.
...
run.bat     # Для Windows
```
Открой браузер на `http://localhost:2004`.
...
```bash
# Поднимет два окна терминала (Бэкенд 2004 и Vite-сервер 5173)
dev.bat
```
